...
This script exports all data from ChromaDB and imports it into Weaviate.
"""
...
class DataMigrator:
    ...
    def _get_weaviate_config(self) -> Dict[str, Any]:
    ...
    def export_chromadb_data(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # This is a limitation of the current interface - we might need to extend it
    ...
    def import_to_weaviate(self, data: List[Dict[str, Any]]) -> None:
        """Import data to Weaviate."""
        logger.info("Starting Weaviate data import...")
        
        try:
            # Create Weaviate instance
            weaviate_client = create_vector_db(self.weaviate_config)
            ...
            logger.info(f"Successfully imported {len(data)} documents to Weaviate")
            
        except Exception as e:
            logger.error(f"Error importing to Weaviate: {e}")
            raise
    ...
    def validate_migration(self) -> bool:
        ...
        try:
            ...
            weaviate_count = weaviate_client.get_collection_info()['count']
            
            logger.info(f"ChromaDB count: {chromadb_count}")
            logger.info(f"Weaviate count: {weaviate_count}")
            
            if chromadb_count == weaviate_count:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Migrate data from ChromaDB to Weaviate")
    ...
    try:
        ...
        if args.dry_run:
            ...
            logger.info("Weaviate config:")
            logger.info(f"  Provider: {migrator.weaviate_config['provider']}")
            logger.info(f"  URL: {migrator.weaviate_config.get('url', 'N/A')}")
            logger.info(f"  Class: {migrator.weaviate_config.get('class_name', 'N/A')}")
            
            # Check if Weaviate is accessible
            try:
                weaviate_client = create_vector_db(migrator.weaviate_config)
                collection_info = weaviate_client.get_collection_info()
                logger.info(f"✅ Weaviate connection successful - Class: {collection_info['name']}")
            except Exception as e:
        ...
        # Perform migration
        logger.info("🚀 Starting migration from ChromaDB to Weaviate...")
        ...
        # Import to Weaviate
        migrator.import_to_weaviate(data)
